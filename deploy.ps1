...
Write-Host "  2. IMPORTANT — Settings -> Root Directory -> set to: backend"
Write-Host "     (If you see 'No start command detected' / Node detected, this is wrong.)"
Write-Host "  3. Settings -> Build -> Builder -> Dockerfile (auto-detected from backend/)"
Write-Host "     If Dockerfile not found, add variable:"
Write-Host "       RAILWAY_DOCKERFILE_PATH=/backend/Dockerfile"
Write-Host "  4. Add env vars:"
Write-Host "       CORS_ORIGINS=https://lawind.ai,https://www.lawind.ai"
Write-Host "       APP_ENV=production"
Write-Host "  5. Settings -> Networking -> Custom Domain -> api.lawind.ai"
Write-Host ""
Write-Host "STEP 3: Deploy frontend (Vercel — https://vercel.com)"
Write-Host "  1. New Project -> Import lawind repo"
Write-Host "  2. IMPORTANT — Root Directory must be 'frontend'"
Write-Host "     (Settings -> General -> Root Directory -> Edit -> frontend)"
Write-Host "     If you see 'No Next.js version detected', this setting is wrong."
